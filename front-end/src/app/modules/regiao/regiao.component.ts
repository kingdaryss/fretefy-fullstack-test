...
export class RegiaoComponent extends BaseFormComponent<Regiao> {
  ...
  constructor(private ibgeService: IbgeService, activatedRoute: ActivatedRoute,
    regiaoService: RegiaoService, formBuilder: FormBuilder, router: Router) {
    super(activatedRoute, formBuilder, regiaoService, router);
    this.citys$ = this.ibgeService.getCitys().pipe( map(items => items.map(item => ({ name: item.nome, id: item.nome }))) );
  }
  protected createFormGroup(response?: Regiao): void {
    this.formGroup = this.formBuilder.group({
      ...
      citys: this.createBuilderCityArray(response)
    });
  }
  private createBuilderCityArray(response?: Regiao): FormArray { return this.formBuilder.array(
    response?.citys?.length ? response.citys.map(city => this.createCityFormGroup(city)) : [this.createCityFormGroup()], [uniqueValidator()]
  )}
